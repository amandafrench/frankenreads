...
class BP_Custom_User_Ids {
 ...
 // SELECT wp_users.id
        ...
        $custom_ids = $wpdb->get_col("SELECT wp-users.id FROM wp_users LEFT JOIN bp_xprofile_data ON (wp_bp_xprofile_data.user_id                    
                        = wp_users.ID) WHERE wp_bp_xprofile_data.field_id = 11 AND wp_users ORDER BY value ASC");
