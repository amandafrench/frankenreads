( function( $, gglcptch ) {
	...
	gglcptch.display = function( container, v1_add_to_last_element, params ) {
		...
		if ( 'v1' == gglcptch_version ) {
			if ( Recaptcha.widget == null || v1_add_to_last_element == true ) {
				Recaptcha.create( gglcptch.options.sitekey, container, { 'theme' : gglcptch.options.theme, 'lang' : gglcptch.options.lang } );
			}
		}
	...
	$( document ).ready( function() {
		...
		$( document ).on( "DOMSubtreeModified", 'form.wpcf7-form .wpcf7-response-output', function() {
			if ( $( this ).text() != '' ) {
